...
def product_error_rate(cross_roads,epsilon_set,experi_times,numofcars):
    ...
    def experiment_in_error(diff):
        ...
        for index_points in range(nums_of_points_set):
            ...
            print 'tranverse:'+str(format_points)
            print 'row:' + str(format_points2)
    diff=[]
    ...
    for i in range(nums_of_points_set):
        experiment_files.write('differ:' + str(diff[i]) + '\n')
        experiment_files.write('average_differ:' + str(AVG_diff[i])+ '\n')
        experiment_files.write('average_differ ratio:' + str(AVG_diff[i]/(numofcars/len(points))) + '\n')
    experiment_files.close()
if __name__ == "__main__":
    epsilon_set = [(x+1)/float(5) for x in range(40)]
    delta = 1.5
    cross_roads = 2
    split =3.4
