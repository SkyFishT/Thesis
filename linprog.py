...
def liner_programming(epsilon=0.1,delta=1.1,global_scope=False):
    ...
    if global_scope==True:
        ...
        for i in crossroad:
            ...
            points_in_scope_file = open(os.path.join(os.getcwd(), 'points_in_scope'+i+'.txt'), 'r')
            points_in_scope_dict=(eval(points_in_scope_file.read()))
            delta_spanner_pairs=np.load(os.path.join(os.getcwd(), 'delta_spanner'+i+'.npy'))
            for index in points_in_scope_dict:
                points_in_scpoe_array_dict[i].append(index)
                for index2 in points_in_scope_dict:
                    if delta_spanner_pairs[index,index2]==1:
                        for k in range(len(points)):
                            tmp_a = [0] * numbers_of_various
                            tmp_a[index * len(points) + k] = 1
                            tmp_a[index2 * len(points) + k] = -ratio
                            a_ub.append(tmp_a)
                            b_ub = b_ub + [0]
        for k in range(len(points)):
            for i in crossroad:
                for j in crossroad:
                    tmp_a = [0] * numbers_of_various
                    for argu_eq_one in points_in_scpoe_array_dict[i]:
                        tmp_a[argu_eq_one*len(points)+k] = 1
                    for argu_eq_epsilon in points_in_scpoe_array_dict[j]:
